    ...
    async (req, accessToken, refreshToken, profile, done) => {
      try {
        ...
        if (role === "student") {
          ...
          if (!instructor) return done(new Error("Instructor not found. You must register manually."), null);

          return done(null, { user: instructor, role });

        } else {
          return done(new Error("Unsupported role or invalid role selection"), null);
        }
