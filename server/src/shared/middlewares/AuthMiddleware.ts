...
export function authenticate(
  ...
  try {
    ...
    req.user = decoded;
    logger.info(
      `Authentication successful for user: ${(decoded as { id?: string })?.id || 'unknown'} from IP: ${req.ip}`,
    );
    next();
  } catch (err) {
    ...
    if (
      ...
      logger.warn(
        `Authentication failed: Invalid or expired token from IP: ${req.ip}, error: ${errorMessage}`,
      );
      throw new HttpError(
      ...
      logger.error(
        `Unexpected error during authentication from IP: ${req.ip}, error: ${errorMessage}`,
      );
      throw new HttpError(
