...
const LessonPlayer: React.FC<LessonPlayerProps> = ({ lessonId, onClose, title }) => {
  ...
  }, [lessonId, signedUrl]); // Re-bind when signedUrl changes (loading finishes)
  ...
  if (isError) {
    return (
      <div className="w-full h-96 bg-gray-900 rounded-xl relative overflow-hidden">
        <ErrorPage
          message={(error as Error)?.message || 'Failed to load lesson'}
          statusCode={403}
        />
        <button 
            onClick={onClose}
