...
export default function LessonItem({ lesson, courseId, moduleId, order, setModules }: Props) {
  ...
  React.useEffect(() => {
    if (JSON.stringify(lesson.resources) !== JSON.stringify(watchedResources)) {
      updateLessonState({ resources: watchedResources as readonly string[] });
    }
  }, [watchedResources, lesson.resources, updateLessonState]);

  const handleEditDiscard = useCallback(() => {
    updateLessonState({
      title: editModule.prevState.lessonTitle,
      description: editModule.prevState.lessonDescription,
    });
          ...
          <button
            onClick={() =>
              setEditLesson({
                disable: false,
                initial: false,
                prevState: { lessonTitle: lesson.title, lessonDescription: lesson.description },
              })
            }
      ...
      ${isDragging ? "border-indigo-500 bg-indigo-50" : "border-gray-300"} ${lesson.isBlocked ? "opacity-50 pointer-events-none" : ""}`}
        ...
        <div className=" mx-auto">
          ...
          <TextInput
            ...
            value={lesson.title}
            onChange={(e) => {
              updateLessonState({ title: e.target.value });
            }}
        ...
        <textarea
          ...
          onChange={(e) => updateLessonState({ description: e.target.value })}
        ...
        <DynamicField
          control={control}
          isDisabled={editModule.disable && !editModule.initial}
          name="resources"
