...
const ContentUploadPage = () => {
  ...
  useEffect(() => {
    ...
    if (!fileNames.length) {
      setModules(data && data.length ? data : [{ moduleId: Date.now().toString(), title: '', description: '', lessons: [] }]);
      return;
    }
  ...
  const addModule = () => {
    setModules((prev) => [
      ...prev,
      { moduleId: Date.now().toString(), title: '', description: '', lessons: [] },
    ]);
  };
    ...
    <div className="min-h-screen dark:bg-gray-900 dark:text-gray-200">
      <div className="max-w-4xl mx-auto p-6">
        ...
        <div className="flex justify-between">
          ...
          <Link to={ROUTES.instructor.myCourses}
