...
import { Search, Filter, Loader2, Users, RefreshCw, Tag, ChevronLeft, ChevronRight } from "lucide-react";
...
const MentorshipBrowsePage = () => {
    ...
    const [selectedSlot, setSelectedSlot] = useState<IMentorshipSlot | null>(null);
    ...
    const lastSlotElementRef = useCallback((node: HTMLDivElement | null) => {
        if (loading) return;
    ...
    const fetchSlots = useCallback(async (pageNum: number, isRefresh: boolean = false) => {
        try {
            setLoading(true);
            ...
            if (isRefresh) {
                ...
                const newSlots = data.filter(slot => !slotIdsRef.current.has(slot.slotId));
                
                // Update Ref
                newSlots.forEach(s => slotIdsRef.current.add(s.slotId));

                // If we got data but it was all duplicates (or empty), stop fetching
                const ended = data.length < ITEMS_PER_PAGE || (data.length > 0 && newSlots.length === 0);
                if (ended) {
                    setHasMore(false);
                }
    ...
    }, [page, debouncedSearch, selectedTags.join(','), minPrice, maxPrice, fromDate, toDate]);
    ...
    const groupedSlots = slots.reduce((acc, slot) => {
        ...
        if (!acc[mentorId]) {
            acc[mentorId] = {
                instructor: slot.instructorDetails || { name: "Expert Instructor", jobTitle: "Industry Specialist"},
                slots: []
            };
        }
        acc[mentorId].slots.push(slot);
        return acc;
    }, {} as Record<string, { instructor: {name: string, jobTitle: string,profilePictureUrl?: string}, slots: IMentorshipSlot[] }>);
                    ...
                    <div className="flex flex-col gap-4 mb-4 mt-8 lg:mt-0">
                        ...
                        {showFilters && (
                            <div className="p-4 bg-gray-50 dark:bg-gray-800/50 rounded-xl border border-gray-200 dark:border-gray-700 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 animate-in slide-in-from-top-2">
                                ...
                                <div>
                                    <label className="text-xs font-semibold text-gray-500 uppercase mb-1 block">Max Price</label>
                                    <input 
                                        type="number" 
                                        placeholder="Max" 
                                        value={maxPrice} 
                            ...
                            >
                                {tagsLoading ? (
                                    ...
                                    Array.from({ length: 8 }).map((_, i) => (
                                        <div key={i} className="h-9 w-24 bg-gray-200 dark:bg-gray-700 rounded-lg animate-pulse shrink-0" />
                                    ))
                                ) : (
                                    categories.map(cat => {
                                        const isActive = cat === 'All' ? selectedTags.length === 0 : selectedTags.includes(cat);
                                        return (
                                            <button
                                                key={cat}
                                            ...
                                            >
                                                {cat === 'All' ? (
                                                    ...
                                                    <Tag size={14} className={isActive ? "text-indigo-200" : "text-indigo-600"} />
                                                )}
                                                {cat}
                                            </button>
                                        );
            ...
            <div className="container mx-auto px-6 py-8">
                ...
                {initialLoading ? (
                    ...
                    <div className="text-center py-20 bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 shadow-sm">
                        <p className="text-gray-500 dark:text-gray-400 text-lg">No mentorship slots found matching your filters.</p>
                        <button 
                            onClick={() => {
                                setSearchTerm("");
                                setSelectedTags([]);
                    ...
                    <>
                        <div className="space-y-12">
                            {Object.entries(groupedSlots).map(([mentorId, data]) => {
                                ...
                                const jobTitle = data.instructor.jobTitle?.split(' ').map((word: string) => word.charAt(0).toUpperCase() + word.slice(1)).join(' ') || 'Industry Specialist';
                                return (
                                    <section key={mentorId} className="animate-in fade-in slide-in-from-bottom-4 duration-700">
                                        <div className="flex items-center gap-4 mb-6 bg-white dark:bg-gray-800 p-4 rounded-2xl border border-gray-100 dark:border-gray-700 shadow-sm">
                                            <div className="w-16 h-16 rounded-full bg-indigo-100 dark:bg-indigo-900/40 flex items-center justify-center text-indigo-600 dark:text-indigo-400 border-2 border-indigo-50 dark:border-indigo-800 overflow-hidden">
                                                {data.instructor.profilePictureUrl ? (
                                                    <img src={data.instructor.profilePictureUrl} alt={data.instructor.name} className="w-full h-full object-cover" />
                                                ) : (
                                                    <span className="text-2xl font-bold">{data.instructor.name.charAt(0).toUpperCase()}</span>
                                                )}
                                            </div>
                                            <div>
                                                <div className="flex items-center gap-1">
                                                    <h2 className="text-xl font-bold text-gray-900 dark:text-white">{data.instructor.name}</h2>
                                                    <svg className="w-4 h-4 text-indigo-600 dark:text-indigo-400" fill="currentColor" viewBox="0 0 20 20">
                                                        <path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clipRule="evenodd" />
                                                    </svg>
                                                </div>
                                                <p className="text-indigo-600 dark:text-indigo-400 font-medium">{jobTitle}</p>
                                                <div className="flex items-center gap-2 mt-1">
                                                    <span className="text-gray-500 dark:text-gray-400 text-xs">{availableSlots.length} available slots</span>
                                                </div>
                                            </div>
                                        </div>
                        ...
                        {hasMore && !loading && <div ref={lastSlotElementRef} className="h-10 w-full" />}
