...
export const BookingCard = ({ booking, onJoinSession, onCancel, userRole }: BookingCardProps) => {
  ...
  const otherParty = userRole === 'instructor' 
    ...
    : (typeof booking.instructorId === 'object' ? booking.instructorId : { name: 'Instructor', profileImageUrl: '', jobTitle: '' });
  ...
  return (
    <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl p-5 shadow-sm">
      <div className="flex justify-between items-start">
        ...
        {booking.cancelledAt && (
            <p className="text-xs text-gray-500 mt-1"> {format(booking.cancelledAt, 'MMM d, yyyy')}</p>
        )}
        {booking.cancelledBy && (
            <p className="text-xs text-gray-500 mt-1 ">Cancelled by <span className="font-bold ">{booking.cancelledBy.charAt(0).toUpperCase() + booking.cancelledBy.slice(1)}</span></p>
        )}
        {booking.completedAt && (
            <p className="text-xs text-gray-500 mt-1">Completed on {format(booking.completedAt, 'MMM d, yyyy')}</p>
        )}
        </div>
      </div>
