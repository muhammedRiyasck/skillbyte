        ...
        <div className="px-6 pb-2">
          <div className="flex flex-col md:flex-row gap-4 mb-4 mt-8 lg:mt-0">
            ...
            >
              ...
              <option value={DateRange.THIRTY_DAYS}>Last 30 Days</option>
              <option value={DateRange.THREE_MONTHS}>Last 3 Months</option>
              ...
              <tbody className="divide-y divide-gray-100 dark:divide-gray-700">
                {purchases.map((purchase) => (
                  <tr key={purchase.id} className="hover:bg-gray-50/50 dark:hover:bg-gray-700/50 transition-colors">
                    ...
                    <td className="px-6 py-5">
                      <span className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold ${purchase.status === PaymentStatus.SUCCEEDED ? 'bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-400' : purchase.status === PaymentStatus.FAILED ? 'bg-red-100 dark:bg-red-900/30 text-red-700 dark:text-red-400' : 'bg-yellow-100 dark:bg-yellow-900/30 text-yellow-700 dark:text-yellow-400'}`}>
                        <span className={`w-1.5 h-1.5 rounded-full animate-pulse ${purchase.status === PaymentStatus.SUCCEEDED ? 'bg-green-600' : purchase.status === PaymentStatus.FAILED ? 'bg-red-600' : 'bg-yellow-400'}`}></span>
